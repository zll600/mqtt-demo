...
interface EnergyAlert {
  type: 'high_usage' | 'low_usage' | 'power_outage' | 'voltage_spike' | 'cost_threshold';
  value: number;
  threshold: number;
...
export class EnergyMeter extends BaseDevice {
  ...
  protected handleCommand(topic: string, payload: any): void {
    console.log(`Energy meter ${this.deviceId} received command:`, { topic, payload });
  ...
  private simulateEnergyConsumption(): void {
    ...
    const powerChange = Math.max(-maxChange, Math.min(maxChange, powerDifference * 0.1));
    
    this.currentReading.instantPower = Math.max(50, this.currentReading.instantPower + powerChange);
    
    // Add some random appliance switching
    if (Math.random() < 0.1) { // 10% chance
      const applianceLoad = this.getRandomApplianceLoad();
      this.currentReading.instantPower += applianceLoad * (Math.random() > 0.5 ? 1 : -1);
      this.currentReading.instantPower = Math.max(50, this.currentReading.instantPower);
    }
  }
  ...
  private updateDerivedValues(): void {
    ...
    const energyIncrement = (this.currentReading.instantPower * timeIntervalHours) / 1000; // Convert to kWh
    this.currentReading.totalEnergy += energyIncrement;
    
    // Update current based on power and voltage
    this.currentReading.current = this.currentReading.instantPower / this.currentReading.voltage;
    ...
    // Calculate cost
    this.currentReading.cost = this.currentReading.totalEnergy * this.costPerKwh;
  }
  ...
  private async publishEnergyReadings(): Promise<void> {
    ...
    await this.publish('frequency', {
      value: this.currentReading.frequency,
      unit: 'hz',
    }, 0);
    
    await this.publish('power_factor', {
  ...
  private async checkForAlerts(): Promise<void> {
    ...
    if (this.currentReading.instantPower > this.highUsageThreshold) {
      alerts.push({
        ...
        severity: this.currentReading.instantPower > this.highUsageThreshold * 1.5 ? 'high' : 'medium',
      });
    }

    // Low usage alert (might indicate power outage or problem)
    ...
    // Voltage spike alert
    if (this.currentReading.voltage > 250 || this.currentReading.voltage < 220) {
      alerts.push({
        type: 'voltage_spike',
        value: this.currentReading.voltage,
        threshold: 240,
        severity: Math.abs(this.currentReading.voltage - 240) > 15 ? 'high' : 'medium',
      });
    }
    ...
    if (monthlyProjectedCost > this.monthlyCostBudget) {
      alerts.push({
        ...
        severity: monthlyProjectedCost > this.monthlyCostBudget * 1.2 ? 'high' : 'medium',
      });
    }
  ...
  private updateDailyUsage(): void {
    ...
    const energyIncrement = (this.currentReading.instantPower * timeIntervalHours) / 1000;
  ...
  private calculateMonthlyProjectedCost(): number {
    ...
    const avgDailyUsage = recentDailyUsage.reduce((sum, day) => sum + day.kwh, 0) / recentDailyUsage.length;
    
    return avgDailyUsage * daysInMonth * this.costPerKwh;
  }

  private async publishDailyUsage(): Promise<void> {
    await this.publish('daily_usage', {
      data: this.dailyUsage.slice(-7), // Last 7 days
      totalWeeklyKwh: this.dailyUsage.slice(-7).reduce((sum, day) => sum + day.kwh, 0),
      averageDailyKwh: this.dailyUsage.slice(-7).reduce((sum, day) => sum + day.kwh, 0) / Math.min(7, this.dailyUsage.length),
    }, 1);
  }
  ...
  private async publishMonthlyStats(): Promise<void> {
    ...
    await this.publish('monthly_stats', {
      totalKwh: monthlyKwh,

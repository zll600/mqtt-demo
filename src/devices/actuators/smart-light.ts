...
export class SmartLight extends BaseDevice {
  ...
  protected handleCommand(topic: string, payload: any): void {
    console.log(`Smart light ${this.deviceId} received command:`, { topic, payload });
  ...
  private simulateGradualChanges(): void {
    ...
    if (Math.random() < 0.05 && this.lightState.on) { // 5% chance
      ...
      if (Math.abs(targetColorTemp - (this.lightState.colorTemperature || 0)) > 100) {
  ...
  private async turnOn(): Promise<void> {
    ...
    if (wasOff) {
      ...
      console.log(`Smart light ${this.deviceId} turned ON (${this.lightState.brightness}%)`);
    }
  }
  ...
  private async turnOff(): Promise<void> {
    ...
    if (wasOn) {
      ...
      console.log(`Smart light ${this.deviceId} turned OFF`);
    }
  }
  ...
  private async setBrightness(brightness: number): Promise<void> {
    ...
    if (this.lightState.brightness !== oldBrightness) {
      ...
      console.log(`Smart light ${this.deviceId} brightness set to ${this.lightState.brightness}%`);
    }
  }

  private async setColor(r: number, g: number, b: number): Promise<void> {
    if (!this.supportsColor) {
      console.log(`Smart light ${this.deviceId} does not support color changes`);
      return;
    }

    const oldColor = this.lightState.color ? { ...this.lightState.color } : null;
    ...
    console.log(`Smart light ${this.deviceId} color set to RGB(${r}, ${g}, ${b})`);
  }

  private async setColorTemperature(temperature: number, isAutomatic: boolean = false): Promise<void> {
    const oldTemp = this.lightState.colorTemperature;
    this.lightState.colorTemperature = Math.max(2700, Math.min(6500, temperature));
    ...
    if (this.lightState.colorTemperature !== oldTemp) {
      ...
      console.log(`Smart light ${this.deviceId} color temperature set to ${this.lightState.colorTemperature}K ${isAutomatic ? '(automatic)' : ''}`);
    }
  }
  ...
  private calculateEfficiency(): number {
    ...
    return Math.round((this.lightState.brightness * 100) / this.lightState.powerConsumption);
  }

...
export class DoorWindowSensor extends BaseDevice {
	...
	protected handleCommand(topic: string, payload: any): void {
		logger.info(
			{ topic, payload },
			`${this.sensorType} sensor ${this.deviceId} received command:`,
		);
	...
	private async changeState(): Promise<void> {
		...
		logger.info(
			`${this.sensorType} ${this.deviceId} changed from ${this.previousState} to ${this.currentState}`,
		);
	...
	private simulateBatteryDrain(): void {
		...
		const stateChangeConsumption =
			this.lastStateChange.getTime() > Date.now() - 60000 ? 0.001 : 0;

		this.batteryLevel = Math.max(
			0,
			this.batteryLevel - baseConsumption - stateChangeConsumption,
		);
	}
	...
	private async publishBatteryLevel(): Promise<void> {
		await this.publish(
			...
			0,
			true,
		); // Retained for latest battery status
	...
	private async publishSecurityAlert(): Promise<void> {
		...
		await this.publish(
			...
			2,
		); // QoS 2 for critical security alerts
	}
	...
	private async performTest(): Promise<void> {
		logger.info(
			`Performing test for ${this.sensorType} sensor ${this.deviceId}`,
		);

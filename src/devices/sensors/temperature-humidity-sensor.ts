...
interface _TemperatureHumidityState {
	temperature: number;
	humidity: number;
	unit: "C" | "F";
}
...
export class TemperatureHumiditySensor extends BaseDevice {
	...
	protected handleCommand(topic: string, payload: any): void {
		logger.info(
			...
			`Temperature sensor ${this.deviceId} received command:`,
		);
	...
	private getAlertSeverity(): "low" | "medium" | "high" {
		...
		const humOut = Math.max(
			0,
			this.currentHumidity - 80,
			20 - this.currentHumidity,
		);

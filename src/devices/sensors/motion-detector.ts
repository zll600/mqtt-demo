...
export class MotionDetector extends BaseDevice {
  ...
  protected handleCommand(topic: string, payload: any): void {
    console.log(`Motion detector ${this.deviceId} received command:`, { topic, payload });

    if (payload.command) {
      switch (payload.command) {
        case 'setSensitivity':
          if (typeof payload.sensitivity === 'number' && payload.sensitivity >= 0 && payload.sensitivity <= 1) {
  ...
  private simulateMotionDetection(): boolean {
    ...
    if (this.room.toLowerCase().includes('living') || this.room.toLowerCase().includes('kitchen')) {
      baseProbability *= 2;
    } else if (this.room.toLowerCase().includes('bedroom') || this.room.toLowerCase().includes('bathroom')) {
      baseProbability *= 0.7;
    }
    ...
    if (timeSinceLastMotion < 30000) { // Less than 30 seconds
      baseProbability *= 0.1;
    }
  ...
  private async triggerMotionDetection(): Promise<void> {
    ...
    const detectedZone = this.zones[Math.floor(Math.random() * this.zones.length)];
    ...
    console.log(`Motion detected in ${this.room} (${this.name}) - Zone: ${detectedZone}, Confidence: ${confidence.toFixed(2)}`);
  }
  ...
  private async clearMotionDetection(): Promise<void> {
    ...
    console.log(`Motion cleared in ${this.room} (${this.name}) - Duration: ${(motionDuration / 1000).toFixed(1)}s`);
  }
  ...
  private setSensitivity(sensitivity: number): void {
    ...
    console.log(`Motion detector ${this.deviceId} sensitivity set to ${this.sensitivity}`);
  }

  private async performTestDetection(): Promise<void> {
    console.log(`Performing test detection for ${this.deviceId}`);
  ...
  private calibrateDetector(): void {
    console.log(`Calibrating motion detector ${this.deviceId}`);

...
export class RuleEngine {
	...
	public onRuleExecution(
		callback: (context: RuleExecutionContext, actions: Action[]) => void,
	): void {
		...
		switch (operator) {
			...
			case "between":
				return (
					...
					Number(actual) <= Number(secondExpected)
				);
			case "in":
	...
	private executeRule(rule: Rule, triggerDevice: DeviceData): void {
		...
		logger.info(
			`🎯 Executing rule: ${rule.name} (triggered by ${triggerDevice.deviceId})`,
		);
		...
		for (const callback of this.executionCallbacks) {
			try {
				...
				logger.error(error, `Error in rule execution callback:`);
			}
		}
	}
		...
		const defaultRules: Rule[] = [
			{
				...
				description:
					"Automatically turn on lights in a room when motion is detected",
				enabled: true,
				priority: 100,
				cooldownMs: 5000,
				conditions: [
			...
			},
			{
				...
				description:
					"Turn off lights after motion clears and some time has passed",
				enabled: true,
				priority: 90,
				cooldownMs: 30000,
				conditions: [
	...
	public getDevices(): Map<string, DeviceData> {

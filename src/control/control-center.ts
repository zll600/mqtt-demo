...
import { RuleEngine, DeviceData, Action, RuleExecutionContext } from './rule-engine.js';
import { controlCenterConfig, buildTopic, topicStructure } from '../config/mqtt-config.js';
...
export class ControlCenter extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.client.on('connect', () => {
          console.log('🏢 Control Center connected to MQTT broker');
        ...
        this.client.on('error', (error) => {
          console.error('Control Center MQTT error:', error);
          this.emit('error', error);
          reject(error);
        });

        this.client.on('close', () => {
          console.log('Control Center disconnected from MQTT broker');
          this.isConnected = false;
          this.emit('disconnected');
        });

        this.client.on('reconnect', () => {
          console.log('Control Center reconnecting to MQTT broker');
    ...
    if (this.client) {
      ...
      return new Promise((resolve) => {
        this.client!.end(false, {}, () => {
          console.log('Control Center disconnected');
          resolve();
        });
      });
    }
  ...
  private subscribeToTopics(): void {
    ...
    this.client.subscribe(deviceDataTopic, { qos: 0 }, (error) => {
      if (error) {
        console.error('Failed to subscribe to device data:', error);
      } else {
        console.log(`📡 Subscribed to device data: ${deviceDataTopic}`);
      }
    });
    ...
    this.client.subscribe(commandTopic, { qos: 1 }, (error) => {
      if (error) {
        console.error('Failed to subscribe to control center commands:', error);
      } else {
        console.log(`🎛️ Subscribed to control commands: ${commandTopic}`);
      }
    });
  }
  ...
  private handleMessage(topic: string, message: Buffer): void {
    try {
      ...
      if (topicParts[0] === topicStructure.home) {
        ...
        } else if (topicParts[1] === 'control-center' && topicParts[2] === 'command') {
  ...
  private handleControlCenterCommand(payload: any): void {
    console.log('🎛️ Control Center received command:', payload);
    ...
    switch (payload.command) {
      ...
      case 'sendDeviceCommand':
        ...
        break;
      default:
        console.log(`Unknown control center command: ${payload.command}`);
    }
  }

  private setupRuleEngineCallbacks(): void {
    this.ruleEngine.onRuleExecution((context: RuleExecutionContext, actions: Action[]) => {
      this.executeRuleActions(context, actions);
    });
  }

  private async executeRuleActions(context: RuleExecutionContext, actions: Action[]): Promise<void> {
  ...
  private async executeAction(action: Action, context: RuleExecutionContext): Promise<void> {
    switch (action.type) {
      case 'device_command':
  ...
  private async executeDeviceCommand(action: Action, context: RuleExecutionContext): Promise<void> {
  ...
  private findRelatedDevice(triggerDevice: DeviceData, action: Action): string | undefined {
    // Find devices in the same room as the trigger device
    const roomDevices = Array.from(this.ruleEngine.getStats().totalDevices > 0 ? [] : [])
      .filter(([_, device]) => device.room === triggerDevice.room);

    // For motion sensors, find lights in the same room
    if (triggerDevice.deviceType.includes('motion')) {
      const light = roomDevices.find(([_, device]) => device.deviceType.includes('light'));
      return light ? light[0] : undefined;
    }

    // For door/window sensors, find lights or motion sensors
    if (triggerDevice.deviceType.includes('door') || triggerDevice.deviceType.includes('window')) {
      const light = roomDevices.find(([_, device]) => device.deviceType.includes('light'));
      return light ? light[0] : undefined;
    }
  ...
  private async sendDeviceCommand(deviceId: string, command: any): Promise<void> {
    if (!this.client) return;
    ...
    this.client.publish(commandTopic, JSON.stringify(commandMessage), { qos: 1 }, (error) => {
      if (error) {
  ...
  private createNotification(action: Action, context: RuleExecutionContext): void {
  ...
  private async executeWebhook(action: Action, context: RuleExecutionContext): Promise<void> {
    ...
    try {
      ...
      console.log(`🔗 Webhook would be called: ${action.webhookUrl}`, webhookData);
    } catch (error) {
      console.error('Webhook execution failed:', error);
    }
  }

  private addNotification(notification: Omit<NotificationMessage, 'id' | 'timestamp' | 'acknowledged'>): void {
  ...
  private acknowledgeNotification(notificationId: string): void {
    ...
    if (notification) {
      ...
      console.log(`✅ Acknowledged notification: ${notification.message}`);
    }
  }
  ...
  private publishRuleStatus(ruleId: string, status: string): void {
    ...
    const statusMessage = {
      ruleId,
      status,
      rule: rule ? { id: rule.id, name: rule.name, enabled: rule.enabled } : null,
      timestamp: new Date().toISOString(),
    };

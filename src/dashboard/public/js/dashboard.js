...
class MQTTDashboard {
    ...
    handleDeviceData(data) {
        ...
        if (device) {
            ...
            if (data.metric === 'power' && data.value && typeof data.value.value === 'number') {
    ...
    createRoomSection(roomName, devices) {
        ...
        const roomTitle = roomName.charAt(0).toUpperCase() + roomName.slice(1).replace('-', ' ');
    ...
    createDeviceCard(device) {
        ...
        col.className = this.currentView === 'grid' ? 'col-md-4 col-sm-6' : 'col-12';
    ...
    getDeviceDisplayData(device) {
        ...
        if (device.deviceType.includes('temperature')) {
            ...
            if (device.data.motion) {
                ...
                const icon = detected ? 'fas fa-eye text-warning' : 'fas fa-eye-slash text-muted';
                html += `<div class="device-metric"><i class="${icon}"></i> ${detected ? 'Motion' : 'Clear'}</div>`;
            }
        } else if (device.deviceType.includes('door') || device.deviceType.includes('window')) {
            if (device.data.state) {
                const state = device.data.state.value.state;
                const icon = state === 'open' ? 'fas fa-door-open text-warning' : 'fas fa-door-closed text-success';
            ...
            if (device.data.state) {
                ...
                const icon = lightState.on ? 'fas fa-lightbulb text-warning' : 'fas fa-lightbulb text-muted';
    ...
    updateControlCenterStatus(status) {
        ...
        if (!status) {
            container.innerHTML = '<div class="text-center text-muted">Control Center Offline</div>';
            return;
        }

        const onlineIcon = status.online ? 'fas fa-check-circle text-success' : 'fas fa-times-circle text-danger';
        const automationIcon = status.automationEnabled ? 'fas fa-play text-success' : 'fas fa-pause text-warning';
    ...
    updateRuleStats(stats) {
        document.getElementById('active-rules').textContent = stats.enabledRules || 0;
    }

    updateStats() {
        const totalDevices = this.devices.size;
        const onlineDevices = Array.from(this.devices.values()).filter(d => d.online !== false).length;
        const unacknowledgedNotifications = this.notifications.filter(n => !n.acknowledged).length;
        ...
        document.getElementById('notification-count').textContent = unacknowledgedNotifications;
    }
    ...
    updateEnergyChart(powerValue) {
        ...
        this.energyChart.data.datasets[0].data = this.energyData.map(d => d.power);
    ...
    openDeviceControl(deviceId) {
        ...
        const modal = new bootstrap.Modal(document.getElementById('deviceControlModal'));
    ...
    generateDeviceControlContent(device) {
        ...
        if (device.deviceType.includes('light')) {
            ...
            content += '<div class="alert alert-info">No specific controls available for this device type.</div>';
        }
    ...
    generateLightControls(device) {
        const lightState = device.data?.state?.value || { on: false, brightness: 50 };
